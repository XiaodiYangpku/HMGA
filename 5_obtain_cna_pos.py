...
#            print(cancer)
        ...
        if int(age)>40:
            if sampleid in sampleid_muts_pos:
                for gene in sampleid_muts_pos[sampleid]:
                    try:
        ...
        for gene in cancer_cya_mut[ctype]:
            ...
            if diff=='NA':
                ...
                if diff>0:
                    ...
                    cya_status='0'
                    diff='%.6f'%abs(diff)
            #with open('tmp.R', 'w') as wf:
            #    wf.write('fisher.test(matrix(c('+str(cya_mut)+','+str(cya_wt)+','+str(oa_mut)+','+str(oa_wt)+'),nrow=2))$p.value\n')
            #pvalue=os.popen('Rscript tmp.R').readline()
...
def main():
    ...
    outputfile1='results/5_cna_pos_csubtype.txt'
    outputfile2='results/5_cna_pos_ctype.txt'
